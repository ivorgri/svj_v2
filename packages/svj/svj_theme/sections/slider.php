<@ if @{ :origUrl } = '/slider' @>
    <section id="@{ :origUrl |
        replace(
            '/\\//',
            ''
        )}" class="h-mobile-slider flex flex-col w-screen text-white mt-mobile-header scroll-mt-mobile-header 
        ...
        <div class="progress-bar w-screen col-span-12 row-span-1 h-[2px]">
            <div id="progress-slider" class="progress-slider h-full bg-svj-green" style="width: var(--progress-slider-width)"></div>
        </div>
        <div class="slider grid grid-cols-12 auto-rows-auto place-items-center h-full overflow-hidden">
            ...
            <@ foreach in pagelist @>
                ...
                transition ease-in-out delay-150 duration-1000">
                    ...
                    <div class="header text-1xl text-svj-green font-semibold col-start-3 col-end-12 row-start-4 row-end-5 z-[5] md:text-2xl md:col-start-3 md:col-end-12 lg:text-3xl">
                        <span class="pb-2 border-b-2 border-svj-green hover:border-b-4 focus:border-b-4 transition duration-200 ease-in-out">
                            <a href="@{ url_slide_link |
                        replace(
                            '/\\//',
                            '#'
                        )}">@{ slide_link_title }</a>
